...
import { Terminal, Square, Trash2, RotateCcw, ArrowLeft, ChevronDown, Send, TerminalSquare, Maximize2, Minimize2 } from 'lucide-react';
...
export default function DirectBotLog() {
  ...
  return (
    <div className="space-y-5 pb-10 max-w-4xl mx-auto">
      ...
      {/* Terminal — rendered via portal in fullscreen so it escapes the layout stacking context */}
      {(() => {
        const terminalCard = (
          <div
            className={
              isFullscreen
                ? 'fixed inset-0 z-[9999] flex flex-col bg-[#050505]'
          ...
          >
            {/* Title bar */}
            <div className="flex items-center justify-between px-4 py-2 border-b border-emerald-500/15 bg-black/60 flex-shrink-0">
              <div className="flex items-center gap-2">
                ...
                {isFullscreen && (
                  <span className="text-[10px] font-mono text-emerald-700 ml-1">· fullscreen</span>
                )}
              </div>
              <div className="flex items-center gap-2">
                {/* Limit selector */}
                <div className="relative">
                  ...
                  {showLimitMenu && (
                    <div className="absolute right-0 top-6 z-[10000] bg-gray-900 border border-gray-700 rounded-lg shadow-xl overflow-hidden min-w-[140px]">
            ...
            <div className="border-t border-emerald-500/15 bg-black/40 px-3 py-2 flex items-center gap-2 flex-shrink-0" title="stdin: sends keypresses to the running bot process | shell: runs a command in the bot directory">
              <button
        ...
        return isFullscreen
          ? createPortal(terminalCard, document.body)
          : terminalCard;
      })()}
